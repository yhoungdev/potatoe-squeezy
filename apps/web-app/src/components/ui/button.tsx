...
const buttonVariants = cva(
  `inline-flex  items-center cursor-pointer justify-center gap-2 whitespace-nowrap
   ...
   dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive`,
  {
    variants: {
      variant: {
        default:
          "bg-orange-600  text-primary-foreground border-none rounded-lg shadow-xs hover:bg-primary/90",
        destructive:
...
function Button({
  ...
  return (
    <Comp
      data-slot="button"
      className={`py-4 ${cn(buttonVariants({ variant, size, className }))}`}
      {...props}
    />
  );
}
